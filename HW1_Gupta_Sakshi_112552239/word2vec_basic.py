...
def generate_batch(data, batch_size, num_skips, skip_window):
  ...
  """
#reference and understanding taken from stackoverflow
  global data_index
  ...
  while a<batch_size:
    m=s+1
    n=s-1
    while n>=s-skip_window and n>=0:
        labels[a]=[data[n]]
        batch[a]=data[s]
        a=a+1
        n=n-1
    while m<=s+skip_window and m<batch_size:
        labels[a]=[data[m]]
        batch[a]=data[s]
        a=a+1
        m=m+1   
...
if __name__ == '__main__':
  ...
  with tf.Session(graph=graph) as sess:
    ...
    pickle.dump([dictionary, trained_steps, embeddings], open(model_filepath, 'wb'),protocol=2)
