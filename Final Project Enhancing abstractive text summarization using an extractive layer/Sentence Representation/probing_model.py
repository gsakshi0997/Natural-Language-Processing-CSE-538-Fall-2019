...
class ProbingClassifier(models.Model):
    def __init__(self,
        ...
        self._classification_layer = layers.Dense(units=classes_num)
    ...
    def call(self, inputs: tf.Tensor, training: bool =False) -> tf.Tensor:
        ...
        layer_representations = self._pretrained_model(inputs, training=False)['layer_representations']
        # print(layer_representations.get_shape().as_list())

        required_representation = layer_representations[:, self._layer_num-1, :]
        # print(required_representation.get_shape().as_list())

        logits = self._classification_layer(required_representation)
