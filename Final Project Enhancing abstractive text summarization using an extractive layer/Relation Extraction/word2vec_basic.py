...
def generate_batch(data, batch_size, num_skips, skip_window):
  ...
  while curLen < batch_size:
    word = data[data_index]

    samples_from_cur = 0
    cur_o = data_index - skip_window

    while (cur_o < data_index and samples_from_cur < num_skips and curLen < batch_size):
    
      batch[curLen] = data[data_index]
      labels[curLen] = data[cur_o]
      
      cur_o += 1
      curLen += 1
      samples_from_cur += 1

    end = data_index + skip_window
    cur_o = data_index + 1

    while (cur_o <= end and samples_from_cur < num_skips and curLen < batch_size) :
      batch[curLen] = data[data_index]
      labels[curLen] = data[cur_o]
...
if __name__ == '__main__':
  ...
  with tf.Session(graph=graph) as sess:
    ...
    # model_filepath = os.path.join(model_path, 'batchsize_%s_embeddingsize_%s_skipwindow_%s_numskips_%s_numsampled_%s_epochs_%s_word2vec_%s.model'%(batch_size, embedding_size, skip_window, num_skips, num_sampled, max_num_steps, loss_model))
